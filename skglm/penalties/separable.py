...
class L1_centered_plus_L2(BasePenalty):
    """:math:`centered ell_1 + ell_2` penalty (the exact penalty is: alpha * \\sum_j (l1_ratio * |w_j - center_j|_1 + (1 - l1_ratio) * w_j^2))."""

    def __init__(self, alpha, l1_ratio, center, positive=False):
        self.alpha = alpha
        self.l1_ratio = l1_ratio
        self.positive = positive
        ...
        if self.positive:
            raise ValueError(
                "L1_centered_plus_L2 penalty does not support positive=True"
            )
        self.center = center  # 1D numpy array, center of the L1 norm |w_j - center_j|_1
    ...
    def value(self, w):
        """Compute the L1 + L2 penalty value."""
        value = self.l1_ratio * self.alpha * np.sum(np.abs(w - self.center))
        value += (1 - self.l1_ratio) * self.alpha / 2 * np.sum(w**2)
        return value

    def prox_1d(self, value, stepsize, j):
        """Compute the proximal operator (scaled soft-thresholding)."""
        weighted_factor = 1 + stepsize * (1 - self.l1_ratio) * self.alpha
        prox = ST(
            value - self.center[j] * weighted_factor,
            self.l1_ratio * self.alpha * stepsize,
            self.positive,
        )
        prox /= weighted_factor
        prox += self.center[j]
        return prox
        ...
        for idx, j in enumerate(ws):
            if self.positive:
                if w[j] < 0:
                    ...
                    # distance of -grad_j to alpha * {l1_ratio + (1 - l1_ratio) * w[j]}
                    subdiff_dist[idx] = np.abs(
                        grad[idx] + alpha * (l1_ratio + (1 - l1_ratio) * w[j])
                    )
            else:
                if self.center[j] == 0:
                    if w[j] == 0:
                        ...
                        subdiff_dist[idx] = np.abs(
                            grad[idx]
                            + alpha * (l1_ratio * np.sign(w[j]) + (1 - l1_ratio) * w[j])
                        )
                else:
                    if w[j] == self.center[j]:
                        subdiff_dist[idx] = max(
                            0,
                            np.abs(grad[idx] + alpha * (1 - l1_ratio) * w[j])
                            - alpha * l1_ratio,
                        )
                    else:
                        subdiff_dist[idx] = np.abs(
                            ...
                            + alpha
                            * (
                                l1_ratio * np.sign(w[j] - self.center[j])
                                + (1 - l1_ratio) * w[j]
                            )
                        )
